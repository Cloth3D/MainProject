...
Sidebar.Material = function ( show ) {

	var signals = show.signals;									// 使用信号槽
	var currentObject;
	...
	var materialName = new UI.Input().setWidth( '150px' ).setFontSize( '12px' ).onChange( function () {
		...
		show.execute( new SetMaterialValueCommand( show.selected, 'name', materialName.getValue() ) );
	...
	function update() {
		...
		if ( material ) {
			...
			if ( material instanceof THREE[ materialClass.getValue() ] === false ) {
				...
				show.execute( new SetMaterialCommand( currentObject, material ), 'New Material: ' + materialClass.getValue() );
			...
			if ( material.color !== undefined && material.color.getHex() !== materialColor.getHexValue() ) {
				...
				show.execute( new SetMaterialColorCommand( currentObject, 'color', materialColor.getHexValue() ) );

			}

			if ( material.roughness !== undefined && Math.abs( material.roughness - materialRoughness.getValue() ) >= 0.01 ) {

				show.execute( new SetMaterialValueCommand( currentObject, 'roughness', materialRoughness.getValue() ) );

			}

			if ( material.metalness !== undefined && Math.abs( material.metalness - materialMetalness.getValue() ) >= 0.01 ) {

				show.execute( new SetMaterialValueCommand( currentObject, 'metalness', materialMetalness.getValue() ) );

			}

			if ( material.emissive !== undefined && material.emissive.getHex() !== materialEmissive.getHexValue() ) {

				show.execute( new SetMaterialColorCommand( currentObject, 'emissive', materialEmissive.getHexValue() ) );

			}

			if ( material.specular !== undefined && material.specular.getHex() !== materialSpecular.getHexValue() ) {

				show.execute( new SetMaterialColorCommand( currentObject, 'specular', materialSpecular.getHexValue() ) );

			}

			if ( material.shininess !== undefined && Math.abs( material.shininess - materialShininess.getValue() ) >= 0.01 ) {

				show.execute( new SetMaterialValueCommand( currentObject, 'shininess', materialShininess.getValue() ) );
			...
			if ( material.vertexColors !== undefined ) {
				...
				if ( material.vertexColors !== vertexColors ) {
					...
					show.execute( new SetMaterialValueCommand( currentObject, 'vertexColors', vertexColors ) );
			...
			if ( material.alphaMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.alphaMap !== alphaMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'alphaMap', alphaMap ) );
			...
			if ( material.bumpMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.bumpMap !== bumpMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'bumpMap', bumpMap ) );

					}

					if ( material.bumpScale !== materialBumpScale.getValue() ) {

						show.execute( new SetMaterialValueCommand( currentObject, 'bumpScale', materialBumpScale.getValue() ) );
			...
			if ( material.normalMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.normalMap !== normalMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'normalMap', normalMap ) );
			...
			if ( material.displacementMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.displacementMap !== displacementMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'displacementMap', displacementMap ) );

					}

					if ( material.displacementScale !== materialDisplacementScale.getValue() ) {

						show.execute( new SetMaterialValueCommand( currentObject, 'displacementScale', materialDisplacementScale.getValue() ) );
			...
			if ( material.roughnessMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.roughnessMap !== roughnessMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'roughnessMap', roughnessMap ) );
			...
			if ( material.metalnessMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.metalnessMap !== metalnessMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'metalnessMap', metalnessMap ) );
			...
			if ( material.specularMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.specularMap !== specularMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'specularMap', specularMap ) );
			...
			if ( material.envMap !== undefined ) {
				...
				if ( material.envMap !== envMap ) {
					...
					show.execute( new SetMaterialMapCommand( currentObject, 'envMap', envMap ) );

				}

				if ( material.reflectivity !== materialReflectivity.getValue() ) {

					show.execute( new SetMaterialValueCommand( currentObject, 'reflectivity', materialReflectivity.getValue() ) );
			...
			if ( material.lightMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.lightMap !== lightMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'lightMap', lightMap ) );
			...
			if ( material.aoMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.aoMap !== aoMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'aoMap', aoMap ) );

					}

					if ( material.aoMapIntensity !== materialAOScale.getValue() ) {

						show.execute( new SetMaterialValueCommand( currentObject, 'aoMapIntensity', materialAOScale.getValue() ) );
			...
			if ( material.emissiveMap !== undefined ) {
				...
				if ( objectHasUvs ) {
					...
					if ( material.emissiveMap !== emissiveMap ) {
						...
						show.execute( new SetMaterialMapCommand( currentObject, 'emissiveMap', emissiveMap ) );
			...
			if ( material.side !== undefined ) {
				...
				if ( material.side !== side ) {
					...
					show.execute( new SetMaterialValueCommand( currentObject, 'side', side ) );
			...
			if ( material.shading !== undefined ) {
				...
				if ( material.shading !== shading ) {
					...
					show.execute( new SetMaterialValueCommand( currentObject, 'shading', shading ) );
			...
			if ( material.blending !== undefined ) {
				...
				if ( material.blending !== blending ) {
					...
					show.execute( new SetMaterialValueCommand( currentObject, 'blending', blending ) );
			...
			if ( material.opacity !== undefined && Math.abs( material.opacity - materialOpacity.getValue() ) >= 0.01 ) {
				...
				show.execute( new SetMaterialValueCommand( currentObject, 'opacity', materialOpacity.getValue() ) );

			}

			if ( material.transparent !== undefined && material.transparent !== materialTransparent.getValue() ) {

				show.execute( new SetMaterialValueCommand( currentObject, 'transparent', materialTransparent.getValue() ) );

			}

			if ( material.alphaTest !== undefined && Math.abs( material.alphaTest - materialAlphaTest.getValue() ) >= 0.01 ) {

				show.execute( new SetMaterialValueCommand( currentObject, 'alphaTest', materialAlphaTest.getValue() ) );

			}

			if ( material.wireframe !== undefined && material.wireframe !== materialWireframe.getValue() ) {

				show.execute( new SetMaterialValueCommand( currentObject, 'wireframe', materialWireframe.getValue() ) );

			}

			if ( material.wireframeLinewidth !== undefined && Math.abs( material.wireframeLinewidth - materialWireframeLinewidth.getValue() ) >= 0.01 ) {

				show.execute( new SetMaterialValueCommand( currentObject, 'wireframeLinewidth', materialWireframeLinewidth.getValue() ) );
	...
	signals.objectSelected.add( function ( object ) {
		...
		if(object instanceof THREE.Group)		// Group的话要选择到孩子才有效
