...
var Show = function() // 建立一个类，容括了render , scene 等东西
{
	...
	this.signals = {//信号
		...
		objectAdded : new SIGNALS.Signal(),//添加模型
		objectChanged : new SIGNALS.Signal(),//变化模型
		objectRemoved : new SIGNALS.Signal(),//移动模型
...
Show.prototype = {
	...
	init : function(width, height, camera_position) {
		...
		this.cameraControl = new THREE.OrbitControls(this.camera,
				this.renderer.domElement); //创建控制器
	...
	execute: function ( cmd, optionalName ) {//操作时，调用execute函数，存储该操作
		this.myHistory.execute( cmd, optionalName );
	}
